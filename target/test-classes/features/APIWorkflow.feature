Feature: API workflow test
  ...
  @api
  Scenario: API test case for creating employee
  ...
  Scenario: Getting the created employee
    ...
    Then the status code for creating an emp is 200
    And the employee id "employee.employee_id" should match with the global emp id
    And the retrieved data at "employee" object should match with the data used for creating the employee
  |emp_firstname|emp_lastname|emp_middle_name|emp_gender|emp_birthday|emp_status|emp_job_title|
  |X123         |Y123        |M              |Male      |2005-01-14  |confirmed |QA Engineer   |


  @api
  Scenario: API test case for creating employee using json body
  ...
  @apidynamic
  Scenario: API test case for creating employee using highly dynamic body
    Given a request is prepared for creating an employee with dynamic data "X123","Y123","M","F","2011-05-12","Confirmed","QA Eng"
    When a POST call is made to create an employee
    Then the status code for creating an employee is 201
    And the response body contains key "Message" and value "Employee Created"
    And the employee id "Employee.employee_id" is stored as global to be used for other requests